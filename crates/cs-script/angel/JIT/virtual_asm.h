...
struct RegPrefix {
    ...
    RegPrefix( Register& Reg, unsigned short Other, bool DefLong ) : reg( Reg ), other( Other ), defLong( DefLong ) { }
};
...
struct CodePage {
    ...
    template <class T> T getFunctionPointer() { return reinterpret_cast<T>( (byte*) page + used ); }
