...
static char* StringCharAt( unsigned int i, string& str ) {
    if ( i >= str.size() ) {
        ...
        ctx->SetException( "Out of range" ); // Set a script exception
...
void RegisterStdString_Native( asIScriptEngine* engine ) {
    ...
    r = engine->RegisterGlobalFunction( "string formatInt(int64 val, const string &in options = \"\", uint width = 0)",
    ...
    assert( r >= 0 );
    r = engine->RegisterGlobalFunction( "uint64 parseUInt(const string &in, uint base = 10, uint &out byteCount = 0)",
