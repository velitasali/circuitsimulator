...
asQWORD CallSystemFunctionNative( asCContext* context, asCScriptFunction* descr, void* obj, asDWORD* args,
        ...
        if ( parmType.IsObject() && !parmType.IsObjectHandle() && !parmType.IsReference() ) {
            ...
            if ( ( parmTypeInfo->flags & COMPLEX_MASK ) ) {
                ...
                if ( !fitsInRegisters ) {
                    ...
                    memcpy( ( (asBYTE*) &argsArray[numArgs] ) + stackDispositionBytes, *(void**) ( args + argsPos ),
                            parmBytes );
...
#if defined( AS_IPHONE ) || defined( AS_MAC )
            {
                ...
                memcpy( ( (asBYTE*) &stackArgs[numStackArgs] ) + stackDispositionBytes, (asQWORD*) &args[argsPos],
                        sizeof( asQWORD ) );
    ...
    if ( IsRegisterHFA( retType ) && !( retTypeInfo->flags & COMPLEX_MASK ) ) {
        ...
        if ( sysFunc->hostReturnSize == 1 )
            *(float*) &retQW = CallARM64Float( gpRegArgs, numGPRegArgs, floatRegArgs, numFloatRegArgs, stackArgs,
                                               numStackArgs, func );
        else
            *(double*) &retQW = CallARM64Double( gpRegArgs, numGPRegArgs, floatRegArgs, numFloatRegArgs, stackArgs,
                                                 numStackArgs, func );
    } else if ( sysFunc->hostReturnInMemory )
        retQW = CallARM64RetInMemory( gpRegArgs, numGPRegArgs, floatRegArgs, numFloatRegArgs, stackArgs, numStackArgs,
                                      retPointer, func );
    else {
        if ( retType.GetSizeInMemoryBytes() > sizeof( asQWORD ) )
            retQW = CallARM64Ret128( gpRegArgs, numGPRegArgs, floatRegArgs, numFloatRegArgs, stackArgs, numStackArgs,
                                     &retQW2, func );
        else
