...
void stdcall allocScriptObject( asCObjectType* type, asCScriptFunction* constructor, asIScriptEngine* engine,
...
#ifdef _MSC_VER
template <class F, class T> void stdcall directConvert( F* from, T* to ) {
    *to = (T) *from;
}
#else
//stdcall doesn't work with templates on GCC
template <class F, class T> void directConvert( F* from, T* to ) {
    *to = (T) *from;
}
#endif
...
enum ObjectPosition { OP_This, OP_First, OP_Last, OP_None };
...
struct SystemCall {
    ...
    SystemCall( Processor& CPU, FloatingPointUnit& FPU, std::function<void( JumpType, bool )> ConditionalReturn,
                asDWORD* const& bytecode, unsigned JitFlags )
...
private:
    ...
    void call_64conv( asSSystemFunctionInterface* func, asCScriptFunction* sFunc, Register* objPointer,
                      ObjectPosition pos );
...
asCJITCompiler::asCJITCompiler( unsigned Flags )
    : activePage( 0 ), lock( new assembler::CriticalSection() ), flags( Flags ), activeJumpTable( 0 ),
      currentTableSize( 0 ) { }
...
int asCJITCompiler::CompileFunction( asIScriptFunction* function, asJITFunction* output ) {
    ...
    *output = activePage->getFunctionPointer<asJITFunction>();
...
#ifdef JIT_64
    //32-bit registers
    Register eax( cpu, EAX ), ebx( cpu, EBX ), ecx( cpu, ECX ), edx( cpu, EDX ), ebp( cpu, EBP, pBits ),
        edi( cpu, R12 );
    //8-bit registers
    ...
    Register pax( cpu, EAX, pBits ), pbx( cpu, EBX, pBits ), pcx( cpu, ECX, pBits ), pdx( cpu, EDX, pBits ),
        esp( cpu, ESP, pBits ), pdi( cpu, R12, pBits ), esi( cpu, R13, pBits );
    ...
    //Pointer-sized registers
    Register pax( cpu, EAX, pBits ), pbx( cpu, EBX, pBits ), pcx( cpu, ECX, pBits ), pdx( cpu, EDX, pBits ),
        esp( cpu, ESP, pBits ), pdi( cpu, EDI, pBits ), esi( cpu, ESI, pBits );
        ...
        if ( pNextOp < end && jumpTable[pNextOp - start] == nullptr ) {
            ...
            if ( pThirdOp < end && jumpTable[pThirdOp - start] == nullptr ) {
                ...
                case asBC_SetV8:
                    if ( thirdOp == asBC_CpyVtoV8
                         && ( nextOp == asBC_ADDd || nextOp == asBC_DIVd || nextOp == asBC_SUBd
                              || nextOp == asBC_MULd ) ) {
                        if ( asBC_SWORDARG0( pOp ) != asBC_SWORDARG2( pNextOp )
                             || asBC_SWORDARG0( pOp ) != asBC_SWORDARG0( pNextOp ) )
                            break;

                        fpu.load_double( *edi
                                         - offset( pNextOp, 1 ) ); //Optimize <Variable Double> <op>= <Constant Double>

                        MemAddress doubleConstant( cpu, &asBC_QWORDARG( pOp ) );

                        switch ( nextOp ) {
                        case asBC_ADDd:
                            fpu.add_double( doubleConstant );
                            break;
                        case asBC_SUBd:
                            fpu.sub_double( doubleConstant );
                            break;
                        case asBC_MULd:
                            fpu.mult_double( doubleConstant );
                            break;
                        case asBC_DIVd:
                            fpu.div_double( doubleConstant );
                            break;
                        default:
                            break;
                        }
                    ...
                    break;
                case asBC_SetV4:
                    if ( nextOp == asBC_SetV4 && thirdOp == asBC_SetV4
                         && asBC_DWORDARG( pOp ) == asBC_DWORDARG( pNextOp )
            ...
            case asBC_CMPIu: {
                ...
                case asBC_JLowNZ:
                    ...
                    break;
                case asBC_JS:
                    jump = isUnsigned ? Below : Sign;
                    break;
                case asBC_JNS:
                    jump = isUnsigned ? NotBelow : NotSign;
                    break;
                case asBC_JP:
                    jump = isUnsigned ? Above : Greater;
                    break;
                case asBC_JNP:
                    jump = isUnsigned ? NotAbove : LessOrEqual;
                    break;
                default:
                    break;
                }
...
#define pushPrep( use )                                                                                                \
    ...
    reservedPushBytes -= use;
        ...
        case asBC_FREE: {
            ...
            if (
                !( objType->flags & asOBJ_REF )
                || !(
                    objType->flags
                    & asOBJ_NOCOUNT ) ) { //Only do FREE on non-reference types, or reference types without fake reference counting
                check_space( 128 );
                ...
                if ( beh->release ) {
                    ...
                    cpu.call_stdcall( (void*) engineDestroyFree, "prp", (asCScriptEngine*) function->GetEngine(), &arg1,
                                      (asCScriptFunction*) function->GetEngine()->GetFunctionById( beh->destruct ) );
                } else if ( objType->flags & asOBJ_LIST_PATTERN ) {
                    as<void*>( *ebp + offsetof( asSVMRegisters, programPointer ) ) = pOp;
                    as<void*>( *ebp + offsetof( asSVMRegisters, stackPointer ) ) = esi;

                    cpu.call_stdcall( (void*) engineListFree, "ppr", (asCScriptEngine*) function->GetEngine(), objType,
                                      &arg1 );
                } else {
                    //Copy over registers to the vm in case the called functions observe the call stack
        ...
        case asBC_RDR4:
            ...
            break;
        case asBC_RDR8:
            as<asQWORD>( *edi - offset0 ).direct_copy( as<asQWORD>( *ebx ), eax );
            break;
        case asBC_LDG:
            pbx = (void*) asBC_PTRARG( pOp );
            break;
        ...
        case asBC_Cast: {
            ...
            asCObjectType* to
                = ( (asCScriptEngine*) function->GetEngine() )->GetObjectTypeFromTypeId( asBC_DWORDARG( pOp ) );
...
void SystemCall::callSystemFunction( asCScriptFunction* func, Register* objPointer, unsigned callFlags ) {
...
#endif
    {
        ...
        case ICC_CDECL:
        case ICC_STDCALL:
            call_64conv( sys, func, 0, OP_None );
            break;
        case ICC_CDECL_OBJLAST:
            call_64conv( sys, func, objPointer, OP_Last );
            break;
        ...
        case ICC_CDECL:
            ...
            break;
        case ICC_STDCALL:
            call_stdcall( sys, func );
            break;
        case ICC_THISCALL:
        case ICC_THISCALL_RETURNINMEM:
            call_thiscall( sys, func, objPointer );
            break;
        case ICC_CDECL_OBJLAST:
            call_cdecl_obj( sys, func, objPointer, true );
            break;
        case ICC_CDECL_OBJFIRST:
            call_cdecl_obj( sys, func, objPointer, false );
            break;
        case ICC_VIRTUAL_THISCALL:
        case ICC_VIRTUAL_THISCALL_RETURNINMEM:
            call_viaAS( func, objPointer );
...
#ifdef JIT_64
void SystemCall::call_64conv( asSSystemFunctionInterface* func, asCScriptFunction* sFunc, Register* objPointer,
                              ObjectPosition pos ) {
    ...
    if ( sFunc->returnType.IsObject() && !sFunc->returnType.IsReference() ) {
        if ( sFunc->returnType.IsObjectHandle() ) {
            ...
            if ( acceptReturn ) {
                ...
                if ( destruct > 0 ) {
                    asCScriptFunction* destructFunc
                        = (asCScriptFunction*) sFunc->GetEngine()->GetFunctionById( destruct );
...
void SystemCall::call_cdecl_obj( asSSystemFunctionInterface* func, asCScriptFunction* sFunc, Register* objPointer,
                                 bool last ) {
...
void SystemCall::call_viaAS( asCScriptFunction* func, Register* objPointer ) {
    ...
    bool isGeneric = func->sysFuncIntf->callConv == ICC_GENERIC_FUNC
        || func->sysFuncIntf->callConv == ICC_GENERIC_FUNC_RETURNINMEM
        || func->sysFuncIntf->callConv == ICC_GENERIC_METHOD
        || func->sysFuncIntf->callConv == ICC_GENERIC_METHOD_RETURNINMEM;
...
void stdcall engineSimpleMethod( asCScriptEngine* engine, void* obj, asSSystemFunctionInterface* i,
                                 asCScriptFunction* func ) {
...
void SystemCall::call_simple( Register& objPointer, asCScriptFunction* func ) {
    ...
    cpu.call_stdcall( (void*) engineSimpleMethod, "prpp", (asCScriptEngine*) func->GetEngine(), &objPointer, sys,
                      func );
}
