...
int ExecuteString( asIScriptEngine* engine, const char* code, void* ref, int refTypeId, asIScriptModule* mod,
    ...
    if ( type )
        type->Release(); // Now it's ok to release the type
    ...
    if ( r >= 0 ) {
        r = execCtx->Execute(); // Execute the function
