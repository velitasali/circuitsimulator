...
impl crate::canvas::Item {
    pub fn half_adder(id: impl Into<String>, x: f64, y: f64, bits: usize) -> Self {
        Self::new(id, x, y, HalfAdder { bits })
    }
}
...
impl HalfAdder {
    pub const TYPE_ID: &'static str = "HalfAdder";
    pub fn to_element_kind(&self) -> Kind {
        Kind::HalfAdder(FullAdderState::new("", self.bits))
    }
...
impl Component for HalfAdder {
    ...
    fn props() -> &'static [PropDef<Self>] {
        ...
        static PROPS: &[PropDef<HalfAdder>] = &[BITS];
        PROPS
    }

    fn pin_geoms(&self) -> Vec<CompPin> {
        half_adder_pins("", self.bits)
            .into_iter()
            .map(CompPin::from)
            .collect()
    }
...
mod tests {
    ...
    #[test]
    fn default_half_adder() {
        let a = HalfAdder::default();
        assert_eq!(a.type_id(), "HalfAdder");
        assert_eq!(a.bits, 4);
        // 4 A + 4 B + 4 S + 1 Co = 13 pins (no Ci)
        assert_eq!(a.pin_geoms().len(), 13);
    }

    #[test]
    fn set_bits() {
        let mut a = HalfAdder::default();
        a.set_prop("Bits", PropValue::Int(1)).unwrap();
        assert_eq!(a.bits, 1);
        // 1 A + 1 B + 1 S + 1 Co = 4 pins
        assert_eq!(a.pin_geoms().len(), 4);
    }
}

fn half_adder_pins(id: &str, bits: usize) -> Vec<Pin> {
    let b = bits.max(1);
    let h = (b * 2 + 2).max(4);
    let half_h = (h / 2) as f64 * 8.0;
    let mut pins = Vec::with_capacity(b * 3 + 1);
    ...
    pins.push(Pin {
        ...
        id: format!("{id}-co"),
        item_id: id.to_string(),
        local: Point::new(20.0, -half_h + 8.0 + (b as f64) * 8.0),
        angle: 0,
        length: 8.0,
        is_bus: false,
        label: "Co".into(),
