...
static void RegisterScriptArray_Native( asIScriptEngine* engine ) {
    ...
    r = engine->RegisterObjectMethod( "array<T>", "int find(uint startAt, const T&in if_handle_then_const value) const",
                                      asMETHODPR( CScriptArray, Find, ( asUINT, void* ) const, int ), asCALL_THISCALL );
    assert( r >= 0 );
    r = engine->RegisterObjectMethod( "array<T>", "int findByRef(const T&in if_handle_then_const value) const",
                                      asMETHODPR( CScriptArray, FindByRef, ( void* ) const, int ), asCALL_THISCALL );
    assert( r >= 0 );
    ...
    r = engine->RegisterObjectMethod(
        ...
        asMETHODPR( CScriptArray, FindByRef, ( asUINT, void* ) const, int ), asCALL_THISCALL );
    assert( r >= 0 );
    ...
    r = engine->RegisterObjectMethod( "array<T>", "void sort(const less &in, uint startAt = 0, uint count = uint(-1))",
                                      asMETHODPR( CScriptArray, Sort, ( asIScriptFunction*, asUINT, asUINT ), void ),
                                      asCALL_THISCALL );
    assert( r >= 0 );

#if AS_USE_STLNAMES != 1 && AS_USE_ACCESSORS == 1
    // Register virtual properties
    r = engine->RegisterObjectMethod( "array<T>", "uint get_length() const property", asMETHOD( CScriptArray, GetSize ),
                                      asCALL_THISCALL );
    assert( r >= 0 );
...
CScriptArray::CScriptArray( asITypeInfo* ti, void* buf ) {
    ...
    if ( !CheckMaxSize( length ) )
        return; // Don't continue with the initialization
...
#if defined( _MSC_VER ) && _MSC_VER >= 1500 && !defined( __S3E__ )
                    sprintf_s( tmp, 512, "Type '%s' does not have a matching opEquals or opCmp method",
                               subType->GetName() );
#else
...
void* CScriptArray::GetDataPointer( void* buf ) {
    if ( ( subTypeId & asTYPEID_MASK_OBJECT ) && !( subTypeId & asTYPEID_OBJHANDLE ) )
        ...
        return buf; // Primitive is just a raw data
}
    ...
    if ( subTypeId & asTYPEID_OBJHANDLE ) {
        ...
        if ( dst->numElements > 0 && src->numElements > 0 ) {
            ...
            if ( subTypeId & asTYPEID_MASK_OBJECT ) {
                ...
                memcpy( dst->data, src->data, count * elementSize ); // Primitives are copied byte for byte
        }
    }
}
...
static void RegisterScriptArray_Generic( asIScriptEngine* engine ) {
    ...
    r = engine->RegisterObjectMethod( "array<T>", "uint length() const", asFUNCTION( ScriptArrayLength_Generic ),
                                      asCALL_GENERIC );
    assert( r >= 0 );
    r = engine->RegisterObjectMethod( "array<T>", "uint size() const", asFUNCTION( ScriptArrayLength_Generic ),
                                      asCALL_GENERIC );
    assert( r >= 0 );
#endif
    r = engine->RegisterObjectMethod( "array<T>", "void reserve(uint length)", asFUNCTION( ScriptArrayReserve_Generic ),
                                      asCALL_GENERIC );
    assert( r >= 0 );
